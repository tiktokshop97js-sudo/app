...
- Atualização (pacote de melhorias): upload de fotos reais no painel (object storage Emergent: POST /api/admin/gallery/upload com JWT, arquivos servidos em /api/files/{path}, soft-delete via db.files); status dos orçamentos (novo/contatado/fechado, PUT /api/admin/leads/{id}, pills coloridas no painel); blindagem de câmera avulsa R$80 (prices.camera, calculadora, detalhe do celular, campo no admin); aviso de tela trincada também na calculadora; selo "50x mais resistente" no hero e na descrição do celular. EMERGENT_LLM_KEY adicionada ao backend/.env para o storage.
